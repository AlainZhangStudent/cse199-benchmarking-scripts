#!/bin/bash
# crash_test.sh
# Maximum combined CPU + GPU stress — instability and failure discovery.
# Uses lib.sh + collectors. This script only orchestrates.
...
echo "Starting GPU stress in Docker..." | tee "$SESSION_FILE"
CONTAINER=$(start_gpu_stress "$TIMESTAMP")
echo "  Container: $CONTAINER"
sleep 5

# ── Live display ──────────────────────────────────────────────────────────────
"$BENCH_DIR/views/temp_display.sh" &
DISPLAY_PID=$!
...
echo "CPU stress done: $(date)" | tee -a "$SESSION_FILE"
