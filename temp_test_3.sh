...
# Start background loggers - suppress startup output
"$BENCH_DIR/temp_benchmark.sh" "$TEMP_FILE" "$STAGE_FILE" > /dev/null 2>&1 &
TEMP_PID=$!

"$BENCH_DIR/power_benchmark.sh" "$POWER_FILE" "$STAGE_FILE" > /dev/null 2>&1 &
POWER_PID=$!

"$BENCH_DIR/cpu_metrics.sh" "$METRICS_FILE" "$STAGE_FILE" > /dev/null 2>&1 &
METRICS_PID=$!
...
# Live display - sleep 3s first to let loggers finish startup
(
    sleep 3
    IIO0=/sys/devices/platform/soc@0/c440000.spmi/spmi-0/0-08/c440000.spmi:pmic@8:adc@3100/iio:device0
...
sudo docker run -d \
    ...
    /bin/bash -c "source /usr/lib/qcom-adreno/qcom-adreno-vars.sh && while true; do dlprim_flops 0:0; done" > /dev/null 2>&1
...
# Kill display BEFORE printing completion to avoid corruption
...
sudo docker stop gpu_stress_${TIMESTAMP} > /dev/null 2>&1
sudo docker rm gpu_stress_${TIMESTAMP} > /dev/null 2>&1
